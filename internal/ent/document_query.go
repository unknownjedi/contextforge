...
func (_q *DocumentQuery) QueryProject() *ProjectQuery {
...
func (_q *DocumentQuery) QuerySource() *SourceQuery {
...
func (_q *DocumentQuery) QueryChunks() *DocumentChunkQuery {
...
func (_q *DocumentQuery) WithProject(opts ...func(*ProjectQuery)) *DocumentQuery {
	query := (&ProjectClient{config: _q.config}).Query()
...
func (_q *DocumentQuery) WithSource(opts ...func(*SourceQuery)) *DocumentQuery {
	query := (&SourceClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *DocumentQuery) WithChunks(opts ...func(*DocumentChunkQuery)) *DocumentQuery {
	query := (&DocumentChunkClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(document.FieldProjectID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Document.Query().
//		Select(document.FieldProjectID).
//		Scan(ctx, &v)
...
func (_q *DocumentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Document, error) {
	...
	if query := _q.withProject; query != nil {
		if err := _q.loadProject(ctx, query, nodes, nil,
			func(n *Document, e *Project) { n.Edges.Project = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withSource; query != nil {
		if err := _q.loadSource(ctx, query, nodes, nil,
			func(n *Document, e *Source) { n.Edges.Source = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withChunks; query != nil {
		if err := _q.loadChunks(ctx, query, nodes,
			...
			func(n *Document, e *DocumentChunk) { n.Edges.Chunks = append(n.Edges.Chunks, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *DocumentQuery) loadProject(ctx context.Context, query *ProjectQuery, nodes []*Document, init func(*Document), assign func(*Document, *Project)) error {
...
func (_q *DocumentQuery) loadSource(ctx context.Context, query *SourceQuery, nodes []*Document, init func(*Document), assign func(*Document, *Source)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Document)
	for i := range nodes {
		fk := nodes[i].SourceID
...
func (_q *DocumentQuery) loadChunks(ctx context.Context, query *DocumentChunkQuery, nodes []*Document, init func(*Document), assign func(*Document, *DocumentChunk)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(documentchunk.FieldDocumentID)
	}
	query.Where(predicate.DocumentChunk(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(document.ChunksColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *DocumentQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withProject != nil {
			_spec.Node.AddColumnOnce(document.FieldProjectID)
		}
		if _q.withSource != nil {
			_spec.Node.AddColumnOnce(document.FieldSourceID)
		}
	}
